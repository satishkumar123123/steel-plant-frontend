...
function Group({group,motors,readings,type,index}){
  ...
  return <article className={'bt-group bt-tone-'+index%6}><header><h3>{group.bridle==='HBR'?'Hot Bridle · HBR':'Bridle '+group.bridle}</h3><span>{group.plant} · A / B</span></header><div className="bt-pair">{pair.map(({position,matches})=>{const motor=matches[0],data=motor&&readings[motor._id],points=data?.points||[];
    return <section className={'bt-motor bt-motor-'+position} key={position}><h4>Motor {position}{matches.length===1&&<Link to={'/motor/'+motor._id}>Details ↗</Link>}</h4>{matches.length===0?<p className="bt-message">Motor not found in directory.</p>:matches.length>1?<p className="bt-message" role="alert">Multiple motors match this position. Check the motor directory.</p>:<><p className="bt-serial">Serial · {motor.serialNo||'Not recorded'}</p>{!data?<p role="status">Loading readings…</p>:data.error?<p className="bt-message" role="alert">{data.error}</p>:!points.length?<p className="bt-message">No valid vibration readings.</p>:<><div className="bt-latest"><strong>{formatValue(points[points.length-1].vibrationValue)} <small>mm/s</small></strong><span>Latest · {date(points[points.length-1].testDate)}</span></div><Chart points={points} type={type} ceiling={ceiling} label={`${group.plant} ${group.bridle} Motor ${position}`}/>{data.excluded>0&&<small>{data.excluded} invalid record(s) excluded.</small>}</>}</>}</section>;
