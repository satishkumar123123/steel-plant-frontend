...
function SafetyWalk(){
  ...
  useEffect(()=>{
    ...
    fetch(`${API}/safety-filter?indicatorType=Leading&category=Safety Walk`)
    .then(res=>res.json())
  ...
  return(
    ...
    <div style={{
      ...
      <h2 style={{
        ...
        Safety Walk Update History
      </h2>

      {history
        .filter(h => h.category === "Safety Walk")
        .map(h=>(
        ...
        >
          ...
          <p><b>Old Area:</b> {h.oldData?.area}</p>

          <p><b>Old Observation:</b> {h.oldData?.observations}</p>
